...
class DestProdVariableDailyLines(models.Model):
    _name = "destination.product.variabledaily.lines"
    _description = "Product Variable Price Line"
    _order = 'price'   
    ...
    @api.depends('product_template_id','membership_id')
    def get_dest_id(self):
        for record in self:
            if record.product_template_id:
                record.dest_id = record.product_template_id.dest_id.id
